...
```{r}
remDr$navigate("https://www.ratemyprofessors.com/search/professors/4303?q=")
```
...
```{r}
remDr$navigate("https://www.ratemyprofessors.com/professor/100888")
```
...
# university 
university <- remDr$findElement(using = "xpath", "//a[@href='/school/4303']")$getElementText() %>%
  unlist()
...
# If [2] is empty or does not contain text, try [1]
if (is.na(review_date) || review_date == "") {
  review_date <- rating$findChildElement(using = "xpath", "(.//div[starts-with(@class, 'TimeStamp')])[1]")$getElementText() %>%
    unlist()
}

# If [1] is empty or does not contain text, try [3]
if (is.na(review_date) || review_date == "") {
  review_date <- rating$findChildElement(using = "xpath", "(.//div[starts-with(@class, 'TimeStamp')])[3]")$getElementText() %>%
    unlist()
}
...
for (t in 1:300) {
  ...
  # Check if the "Show More" button was found
  if (isTRUE(show_more$isElementDisplayed())) {
